...
class DBOperations(ABC):
    def __init__(self):
       self.con = pyodbc.connect("DRIVER={SQL SERVER};SERVER=LAPTOP-GRA1721A\SQLEXPRESS;database=ProjectDBCAS;username=sa;pwd=mydatabase")

class DBLogin(DBOperations):
    def __init__(self):
        DBOperations.__init__(self)

    def getImage(self, name):
        cur = self.con.cursor()
        query = "Select image from LoginImage where name = ?"
...
class DBStudents(DBOperations):
    ...
    def UpdateStudent(self, stud, rid):
        cur = self.con.cursor()  

        query = "Update Students set Name =?, DOB =?, Gender=?, FatherName=?, MotherName=?, Address=?, City=?, ContactNo = ?, EmailId=?, Nationality=? where RegistrationId = ?"
        row = (stud.Name, stud.Dob, stud.Gender, stud.FatherName, stud.MotherName, stud.Address, stud.City, stud.ContactNo, stud.EmailId, stud.Nationality, rid)

        cur.execute(query,row)
        self.con.commit()

    def DeleteStudent(self, rid):
        cur = self.con.cursor()

        cur.execute("delete from Marks where RegistrationId =?", rid)
        cur.execute("delete from Attendance where RegistrationId =?", rid)
        cur.execute("delete from Enrollments where RegistrationId = ?", rid)
        cur.execute("delete from Students where RegistrationId =?", rid)
        self.con.commit()
...
class DBMarks(DBOperations):
    ...
    def AddMarks(self, ssid, rid, marks):
        cur = self.con.cursor()
